...
Важно: конфиг загружается в режиме **fail-fast**.

- отсутствует обязательная секция/поле -> `load_server_config` завершится ошибкой;
- отсутствует обязательный параметр профиля -> падение при инициализации профиля;
- невалидное состояние профиля в `server_state.yaml` -> падение при восстановлении.

В проекте больше не используются «тихие» подстановки дефолтов для обязательных полей.
